...
ReadProcessMemory.argtypes = [w.HANDLE, w.LPCVOID,
...
class TekkenGameReader:
    ...
    def GetValueFromAddress(self, processHandle, address, isFloat=False, is64bit=False, isString=False):
        ...
        successful = ReadProcessMemory(processHandle, address, c.byref(
    ...
    def GetValueAtEndOfPointerTrail(self, processHandle, data_type, isString):
        ...
        for i, offset in enumerate(addresses):
            if i + 1 < len(addresses):
                value = self.GetValueFromAddress(
                    processHandle, value + offset, is64bit=True)
            else:
                value = self.GetValueFromAddress(
                    processHandle, value + offset, isString=isString)
        return value
...
#                print("Unrecognized location for TekkenGame-Win64-Shipping.exe module. Tekken.exe Patch? Wrong process id?")
        ...
        if self.module_address != None:
            ...
            try:
                addresses = list(
                    map(to_hex, self.player_data_pointer_offset.split()))
                value = self.module_address
                for i, offset in enumerate(addresses):
                    if i + 1 < len(addresses):
                        value = self.GetValueFromAddress(
                ...
                if player_data_base_address == 0:
                    ...
                    player_data_frame = self.GetBlockOfData(
                        processHandle, player_data_second_address, self.c['MemoryAddressOffsets']['rollback_frame_offset'])

                    for data_type, value in self.c['PlayerDataAddress'].items():
                        p1_value = self.GetValueFromDataBlock(
                            player_data_frame, value, 0, self.IsDataAFloat(data_type))
                        p2_value = self.GetValueFromDataBlock(
                            player_data_frame, value, self.c['MemoryAddressOffsets']['p2_data_offset'], self.IsDataAFloat(data_type))
                        p1_bot.player_data_dict['PlayerDataAddress.' +
                                                data_type] = p1_value
                        p2_bot.player_data_dict['PlayerDataAddress.' +
                                                data_type] = p2_value
                    ...
                    for data_type, value in self.c['EndBlockPlayerDataAddress'].items():
                        ...
                        p2_value = self.GetValueFromDataBlock(
                            player_data_frame, value, self.c['MemoryAddressOffsets']['p2_end_block_offset'])
                        p1_bot.player_data_dict['EndBlockPlayerDataAddress.' +
                                                data_type] = p1_value
                        p2_bot.player_data_dict['EndBlockPlayerDataAddress.' +
                                                data_type] = p2_value

                    bot_facing = self.GetValueFromDataBlock(
                        player_data_frame, self.c['GameDataAddress']['facing'])
                    timer_in_frames = self.GetValueFromDataBlock(
                        player_data_frame, self.c['GameDataAddress']['timer_in_frames'])
                    ...
                    for axis, startingAddress in ((k, v) for k, v in self.c['PlayerDataAddress'].items() if k in ('x', 'y', 'z')):
                        ...
                        p2_coord_array = []
                        for i in range(23):
                            p1_coord_array.append(self.GetValueFromDataBlock(
                                player_data_frame, startingAddress + (i * positionOffset), 0, is_float=True))
                            p2_coord_array.append(self.GetValueFromDataBlock(player_data_frame, startingAddress + (
                                i * positionOffset), self.c['MemoryAddressOffsets']['p2_data_offset'], is_float=True))
                        p1_bot.player_data_dict['PlayerDataAddress.' +
                                                axis] = p1_coord_array
                        p2_bot.player_data_dict['PlayerDataAddress.' +
                                                axis] = p2_coord_array
                    ...
                    p1_bot.player_data_dict['use_opponent_movelist'] = p1_bot.player_data_dict[
                        'PlayerDataAddress.movelist_to_use'] == self.p2_movelist_to_use
                    p2_bot.player_data_dict['use_opponent_movelist'] = p2_bot.player_data_dict[
                        'PlayerDataAddress.movelist_to_use'] == self.p1_movelist_to_use
                    ...
                    if self.flagToReacquireNames:
                        if p1_bot.character_name != CharacterCodes.NOT_YET_LOADED.name and p2_bot.character_name != CharacterCodes.NOT_YET_LOADED.name:
                            self.opponent_name = self.GetValueAtEndOfPointerTrail(
                                processHandle, "OPPONENT_NAME", True)
                            self.opponent_side = self.GetValueAtEndOfPointerTrail(
                                processHandle, "OPPONENT_SIDE", False)
                            self.is_player_player_one = (
                                self.opponent_side == 1)
                            # print(self.opponent_char_id)
                            # print(self.is_player_player_one)

                            self.p1_movelist_to_use = p1_bot.player_data_dict[
                                'PlayerDataAddress.movelist_to_use']
                            self.p2_movelist_to_use = p2_bot.player_data_dict[
                                'PlayerDataAddress.movelist_to_use']

                            self.p1_movelist_block, p1_movelist_address = self.PopulateMovelists(
                                processHandle, "P1_Movelist")
                            self.p2_movelist_block, p2_movelist_address = self.PopulateMovelists(
                                processHandle, "P2_Movelist")

                            self.p1_movelist_parser = MovelistParser.MovelistParser(
                                self.p1_movelist_block, p1_movelist_address)
                            self.p2_movelist_parser = MovelistParser.MovelistParser(
                            ...
                            self.p1_movelist_names = self.p1_movelist_block[0x2E8:200000].split(
                                b'\00')  # Todo: figure out the actual size of the name movelist
                            self.p2_movelist_names = self.p2_movelist_block[0x2E8:200000].split(
                                b'\00')
                    ...
                    gameSnapshot = GameSnapshot(
                        p1_bot, p2_bot, best_frame_count, timer_in_frames, bot_facing, self.opponent_name, self.is_player_player_one)
    ...
    def PopulateMovelists(self, processHandle, data_type):
        ...
        movelist_address = self.GetValueFromAddress(
            processHandle, self.module_address + movelist_trail[0], is64bit=True)
        movelist_block = self.GetBlockOfData(
            processHandle, movelist_address, self.c["MemoryAddressOffsets"]["movelist_size"])
...
class BotSnapshot:
    ...
    def Bake(self):
        ...
        self.simple_state = SimpleMoveStates(
            d['PlayerDataAddress.simple_move_state'])
        ...
        self.complex_state = ComplexMoveStates(
            d['PlayerDataAddress.complex_move_state'])
        ...
        self.input_direction = InputDirectionCodes(
            d['PlayerDataAddress.input_direction'])
        self.input_button = InputAttackCodes(
            d['PlayerDataAddress.input_attack'] % InputAttackCodes.xRAGE.value)
        ...
        self.is_cancelable = (CancelStatesBitmask.CANCELABLE.value &
                              cancel_window_bitmask) == CancelStatesBitmask.CANCELABLE.value
        self.is_bufferable = (CancelStatesBitmask.BUFFERABLE.value &
                              cancel_window_bitmask) == CancelStatesBitmask.BUFFERABLE.value
        self.is_parry_1 = (CancelStatesBitmask.PARRYABLE_1.value &
                           cancel_window_bitmask) == CancelStatesBitmask.PARRYABLE_1.value
        self.is_parry_2 = (CancelStatesBitmask.PARRYABLE_2.value &
                           cancel_window_bitmask) == CancelStatesBitmask.PARRYABLE_2.value

        self.is_recovering = (ComplexMoveStates.RECOVERING.value &
                              recovery_window_bitmask) == ComplexMoveStates.RECOVERING.value
        ...
        self.skeleton = (d['PlayerDataAddress.x'],
                         d['PlayerDataAddress.y'], d['PlayerDataAddress.z'])

        self.active_xyz = (d['PlayerDataAddress.activebox_x'],
                           d['PlayerDataAddress.activebox_y'], d['PlayerDataAddress.activebox_z'])
        ...
        try:
            self.character_name = CharacterCodes(
                d['PlayerDataAddress.char_id']).name
        except:
            self.character_name = "UNKNOWN"

    def PrintYInfo(self):
        # print("h: " + str(self.highest_y) + " l: " + str(self.lowest_y) + ' d: ' + str(self.highest_y - self.lowest_y))
        print('{:.4f}, {:.4f}, {:.4f}'.format(self.highest_y,
              self.lowest_y, self.highest_y - self.lowest_y))
    ...
    def IsWallSplat(self):
        # TODO: use the wall splat states in ComplexMoveStates #move ids may be different for 'big' characters
        return self.move_id == 2396 or self.move_id == 2387 or self.move_id == 2380 or self.move_id == 2382
...
class TekkenGameState:
        ...
        if (gameData != None):
            ...
            if len(self.stateLog) == 0 or gameData.frame_count != self.stateLog[-1].frame_count:
                ...
                if len(self.stateLog) > 0:
                    frames_lost = gameData.frame_count - \
                        self.stateLog[-1].frame_count - 1
                    if frames_lost > 0:
                        pass
                ...
                for i in range(min(7 - buffer, frames_lost)):
                    ...
                    droppedState = self.gameReader.GetUpdatedState(
    ...
    def BackToTheFuture(self, frames):
        if self.futureStateLog != None:
            raise AssertionError(
                'Already called BackToTheFuture, need to return to the present first, Marty')
        else:
    ...
    def IsBotGettingHit(self):
        # or self.GetFramesSinceBotTookDamage() < 15
    ...
    def GetOppMoveInterruptedFrames(self):
        if len(self.stateLog) > 3:
            if self.stateLog[-1].opp.move_timer == 1:
                interruptedFrames = self.stateLog[-2].opp.move_timer - (
                    self.stateLog[-3].opp.move_timer + 1)
    ...
    def GetCurrentOppMoveString(self):
        if self.stateLog[-1].opp.movelist_parser != None:
            ...
            while (True):
                next_move, last_move_was_empty_cancel = self.GetOppMoveString(
    ...
    def GetOppTechnicalStates(self, startup):
        ...
        unparryable = MoveDataReport(
            'NO PARRY?', [not parryable1.is_present() and not parryable2.is_present()])
    ...
    def GetRangeOfMove(self):
        ...
        unit_vector_towards_bot = (
            vector_towards_bot[0]/toward_bot_magnitude, vector_towards_bot[1]/toward_bot_magnitude)

        movements = [(ai_x - bi_x, ai_z - bi_z)for ai_x, bi_x, ai_z, bi_z in zip(
            ending_skeleton[0], starting_skeleton[0], ending_skeleton[2], starting_skeleton[2])]
        dotproducts = []
        for movement in movements:
            dotproducts.append(
                movement[0] * unit_vector_towards_bot[0] + movement[1] * unit_vector_towards_bot[1])
