...
# ======================================= #
        if self.gameplan != None:
            distance = gameState.GetDist()
            if (distance <= 2000):
                if self.botCommands.IsAvailable():
                    if (self.botOptions.blockType == "Defend All"):
                        BotBehaviors.DefendAllAttacks(
                            gameState, self.botCommands, blockLowType=self.botOptions.blockLowType)
